...
export class WorkerController {
  ...
  public findAllWorkers = asyncHandler(async (req, res, next) => {
    try {
      ...
      const workerData: Worker[] =
        await this.workerService.findAllWorkers(searchTerm as string);
      res
        .status(200)
        .json({
          success: true,
  ...
  public findWorkerById = asyncHandler(async (req, res, next) => {
    try {
      const { id } = req.params;
      const workerData: Worker =
        await this.workerService.findWorkerById(id);
  ...
  public updateWorker = asyncHandler(async (req, res, next) => {
    try {
      ...
      const updatedWorkerData: Worker =
        await this.workerService.updateWorker(workerData);
  ...
  public deleteWorkerById = asyncHandler(async (req, res, next) => {
    try {
      const { id } = req.params;
      const workerData: Worker =
        await this.workerService.deleteWorkerById(id);
